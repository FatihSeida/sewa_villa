...
import 'package:sewa_villa/models/villa_model.dart';
...
class ProfileMatching {
  ...
  double hitungPoin(int harga, int jarak, int kapasitas, int fasilitas) {
    ...
    if (kapasitas <= 0) {
      pKapasitas = 0;
    } else if (kapasitas <= 10 && kapasitas > 0) {
      pKapasitas = 1;
    } else if (kapasitas <= 15 && kapasitas > 10) {
      pKapasitas = 2;
    } else if (kapasitas <= 20 && kapasitas > 15) {
      pKapasitas = 3;
    } else if (kapasitas <= 25 && kapasitas > 20) {
      pKapasitas = 4;
    } else if (kapasitas <= 30 && kapasitas > 25) {
      pKapasitas = 5;
    } else if (kapasitas <= 35 && kapasitas > 30) {
      pKapasitas = 6;
    } else if (kapasitas <= 40 && kapasitas > 35) {
      pKapasitas = 7;
    } else if (kapasitas <= 45 && kapasitas > 40) {
      pKapasitas = 8;
    } else if (kapasitas <= 50 && kapasitas > 45) {
      pKapasitas = 9;
    } else {
      pKapasitas = 10;
    }
  ...
  Future<List<IndexPoint>> perhitungan() async {
    ...
    for (Villa villa in datas) {
      fasilitas = villa.fasilitasLength;
      currentHargaBanding = currentHarga - villa.harga;
      currentKapasitasBanding = villa.kapasitas - currentKapasitas;
      // menghitung jarak berdasar koordinat saat ini dan koordinat villa
      int jarak = hitungJarak(
          currentLatitude, currentLongitude, villa.latitude, villa.longitude);
      double poin = hitungPoin(
          currentHargaBanding, jarak, currentKapasitasBanding, fasilitas);
      villaDatas.add(IndexPoint(villa, jarak, poin, villa.harga));
    }
